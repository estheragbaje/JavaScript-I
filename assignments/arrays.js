...
for (let i = 0; i < inventory.length; i++) {
  ...
  if (car.id === 33) {
    console.log(
      `Car 33 is a ${car.car_make} ${car.car_model} ${car.car_year} `
    );
  }
}
...
lastCar = inventory[lastCarIndex];
...
//loop to check through all the carYears
for (let i = 0; i < carYears.length; i++) {
  //store each item in a new variable
  let carYear = carYears[i];

  //condition from question
  if (carYear < 2000) {
    //pushing carYear into oldCars array
...
for (let i = 0; i < inventory.length; i++) {
  // store each carmake into carMake
  let inventoryItem = inventory[i];
  let carMake = inventoryItem.car_make;
